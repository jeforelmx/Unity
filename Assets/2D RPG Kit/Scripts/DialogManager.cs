...
public class DialogManager : MonoBehaviour {
    ...
    private List<DialogChoiceButton> dialogChoiceButtons;
	...
	void Update () {
        ...
        if(dialogBox.activeInHierarchy)
        {
            ...
            {
                Debug.Log("dialog press confirm!!");
                ...
                if (!justStarted)
                {
                    ...
                    if (currentLine >= dialogLines.Length)
                    {
                        if (dialogStarter.dialogChoices.Count == 0)//choiceA == null && choiceB == null)
                        {
                            ...
                            {
                                GameMenu.instance.gotItemMessageText.text = "得到 " + Shop.instance.selectedItem.name + " 辣!";
                            ...
                            if (itemGiven)
                            {
                                GameMenu.instance.gotItemMessageText.text = "失去 " + Shop.instance.selectedItem.name + "!";
                                StartCoroutine(gotItemMessageCo());
                                itemGiven = false;
                            }
                        ...
                        {
                            //if (dialogChoicesIsShow)
                            //{
                            //    Debug.Log("dialog choices is already show!");
                            //    for (int i = 0; i < dialogChoiceButtons.Count; ++i)
                            //    {
                            //        if(dialogChoiceButtons[i].GetComponent<Button>() == EventSystem.current.currentSelectedGameObject)
                            //        {
                            //            Debug.Log("select button!");
                            //            SelectDialogChoice(dialogChoiceButtons[i].index);
                            //            dialogChoiceButtons.Clear();
                            //        }
                            //    }
                            //}
                            //else
                            {
                                ...
                                Debug.Log("dialog choices is show!");
                                //GameMenu.instance.btn = choiceButton;
                                //GameMenu.instance.SelectFirstButton();
                                dialogChoiceButtons = new List<DialogChoiceButton>();
                                for (int i = 0; i < dialogStarter.dialogChoices.Count; i++)
                                {
                                    ...
                                    dialogChoiceButtons.Add(choiceButton.GetComponent<DialogChoiceButton>());
    ...
    public void ShowDialog(Sprite[] portraits, string[] newLines, bool isPerson)
    {
        ...
        Debug.Log("dialog choices is close!");
        ...
        if (newLines.Length == 0)
        {
            ...
            if (itemGiven)
            {
                GameMenu.instance.gotItemMessageText.text = "失去 " + Shop.instance.selectedItem.name + " 辣!";
                StartCoroutine(gotItemMessageCo());
                itemGiven = false;
            }
            ...
            if (fullInventory)
            {
                Shop.instance.promptText.text = "发现 " + Shop.instance.selectedItem.name + "." + "\n" + "但是背包已经满了!";
                StartCoroutine(Shop.instance.PromptCo());
                fullInventory = false;
            }
    ...
    public void CheckIfName()
    {
        ...
        {
            string text = dialogLines[currentLine].Replace("//", "");
            if(text == "Rem" || text == "rem")
                text = PlayerController.instance.playername;
            nameText.text = text;
            currentLine++;
        }
        else
    ...
    public void SelectDialogChoice(int buttonValue)
    {
        ...
        //}
        if (dialogStarter.dialogChoices[buttonValue].choiceEvent == null)
