...
public class GameMenu : MonoBehaviour {
    ...
    public void ShowSkills(int charNumber)
    {
        ...
        for (int i = 0; i < skillButtons.Length; i++)
        {
            if (playerStats[charNumber].skills.Length > i)
            {
                ...
                skillButtons[i].skill = playerStats[charNumber].skills[i].skill.skillName;
                ...
                {
                    if (BattleManager.instance.skillList[j].skill.skillName == skillButtons[i].skill)
                    {
                        skillButtons[i].skillCost = BattleManager.instance.skillList[j].skill.skillCost;
    ...
    public void ShowItems()
    {
        ...
        {            
            ...
            if(GameManager.instance.itemsHeld[i] != "")
            {
                ...
                itemButtons[i].amountText.text = GameManager.instance.itemsHeld[i];// GameManager.instance.GetItemDetails(GameManager.instance.itemsHeld[i]).itemName;
            } else
            {
    ...
    public void SelectEquipItem(Item newItem)
    {
        if (newItem != null)
        {
            ...
            if (activeItem.item)
            {
                equipUseButtonText.text = "使用";
            }
            if (activeItem.offense || activeItem.defense)
            {
                equipUseButtonText.text = "装备";
            }
